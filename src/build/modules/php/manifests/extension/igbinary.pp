class php::extension::igbinary {
  ...
  exec { '/bin/bash -l -c "cd /tmp/igbinary-1.2.1 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.2.17 --enable-igbinary"':
    timeout => 0,
  ...
  exec { '/bin/bash -l -c "cd /tmp/igbinary-1.2.1 && make"':
    timeout => 0,
    require => Exec['/bin/bash -l -c "cd /tmp/igbinary-1.2.1 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.2.17 --enable-igbinary"']
  }
