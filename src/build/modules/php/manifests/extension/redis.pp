class php::extension::redis {
  ...
  bash_exec { 'cd /tmp/redis-2.2.5 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.2.17 --enable-redis-igbinary':
    timeout => 0,
  ...
  bash_exec { 'cd /tmp/redis-2.2.5 && make':
    timeout => 0,
    require => Bash_exec['cd /tmp/redis-2.2.5 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.2.17 --enable-redis-igbinary']
  }
