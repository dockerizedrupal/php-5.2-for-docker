class php::extension::apcu {
  ...
  bash_exec { 'cd /tmp/apcu-4.0.7 && ./configure --with-php-config=/usr/local/bin/phpfarm/inst/bin/php-config-5.2.17':
    timeout => 0,
  ...
  bash_exec { 'cd /tmp/apcu-4.0.7 && make':
    timeout => 0,
    require => Bash_exec['cd /tmp/apcu-4.0.7 && ./configure --with-php-config=/usr/local/bin/phpfarm/inst/bin/php-config-5.2.17']
  }
